...
Start the Shepherd autopilot loop. Reads and executes tasks from TODO-Today.md autonomously.
...
while .autopilot contains "run":
    ...
    # Save context
    run /sh:handoff to produce handoff document

    # Check retro
    if retro counter >= 10: run /sh:retro before next task
...
- Context is saved via `/sh:handoff` at every session boundary
- Retros happen on schedule via `/sh:retro` (every 10 US)
