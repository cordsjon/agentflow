...
7. **Spec review loop** — invoke `/sh:spec-review` on the written spec; fix issues and re-review until approved (max 5 iterations, then surface to human)
8. **User reviews written spec** — ask user to review the spec file before proceeding
9. **Transition to implementation** — invoke `/sh:plan` to create implementation plan
...
1. Invoke `/sh:spec-review` on the spec file
...
- Invoke `/sh:plan` to create a detailed implementation plan
- Do NOT invoke any other skill. `/sh:plan` is the next step.
